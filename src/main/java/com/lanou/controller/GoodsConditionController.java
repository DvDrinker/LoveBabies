...
//  要接受基本数据类型的集合，使用数组接收即可
    ...
    public String createTable(Integer[] goods_ids,HttpServletResponse response){
...
//
                    ...
                    if (!biggerConditionMap.get(gc.getCondition_name()).containsKey(gc.getCondtion_value())){
                        biggerConditionMap.get(gc.getCondition_name()).put(gc.getCondtion_value(),1);
                    }
                    else{
                       Integer a =  biggerConditionMap.get(gc.getCondition_name()).get(gc.getCondtion_value());
                       a++;
                       biggerConditionMap.get(gc.getCondition_name()).put(gc.getCondtion_value(),a);
                    }
                }
